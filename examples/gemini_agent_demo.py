"""
Google Gemini Agent Demo implementing DAWN architecture.

This demo showcases the use of Google Gemini models within the DAWN framework.
It demonstrates various capabilities of the GeminiAgent.
"""
import sys
import os
import json
...
from src.gemini_agent import create_gemini_agent, GeminiAgent
...
def check_environment():
    ...
    if not APIConfig.is_gemini_configured():
        print("Error: Google Gemini API is not configured.")
...
def text_generation_demo(agent: GeminiAgent):
    """Demonstrate text generation capability."""
    display_divider("Text Generation Demo")
    
    prompt = "Write a short poem about artificial intelligence and distributed agents."
...
def chat_demo(agent: GeminiAgent):
...
def summarization_demo(agent: GeminiAgent):
...
def classification_demo(agent: GeminiAgent):
...
def extraction_demo(agent: GeminiAgent):
...
def run_demo():
    """Run the Gemini agent demo."""
    print("=== Google Gemini Agent Demo (DAWN Framework) ===\n")
    ...
    agent = create_gemini_agent(
        ...
        model="gemini-1.5-pro",  # You can change this to other Gemini models
        temperature=0.3,
        max_tokens=1024,
    ...
    if not health_status:
        print("Agent health check failed. Please check your Gemini API configuration.")
        sys.exit(1)
